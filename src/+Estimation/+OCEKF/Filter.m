classdef Filter < handle
    ...
    methods
        ...
        function obj = setup(obj, prior_dev, sensors_dev, model_comp, options)
            ...
            obj.P0 = Estimation.OCEKF.State.constructVar(prior_dev, options.enable_bias_estimation);
        ...
        function obj = initialize(obj, x0, ba0, bg0)
            if (obj.filter_configured)
                if (obj.options.enable_bias_estimation)
                    ...
                    ba_range = Estimation.OCEKF.State.getRange('bias_acc', state_type, obj.options.enable_bias_estimation);
                    bg_range = Estimation.OCEKF.State.getRange('bias_gyro', state_type, obj.options.enable_bias_estimation);
                    obj.ba0 = x0(ba_range);
                    obj.bg0 = x0(bg_range);
                else
        ...
        function [x, P, BaseLinkState, DebugOut, obj] = advance(obj, t, omega, alpha, encoders, contacts)
            ...
            [q, p, v, ~, ~, ~, ~, ~, bg] = Estimation.OCEKF.State.extract(obj.x, obj.ba0, obj.bg0);
    ...
    methods (Access = private)
        function obj = predictState(obj, omega, alpha, encoders, contact, dt)
            [q, p, v, qlf, plf, qrf, prf, ba, bg] = Estimation.OCEKF.State.extract(obj.x, obj.ba0, obj.bg0);
            R = Utils.quat2rot(q);
            ...
            if (abs(norm(q_pred) - 0.0) < 10e-3)
                disp('[WARNING][OCEKF::predictState] Malformed quaternion, returning previous state');
                return;
            end
            ...
            %- not be activated
            [q_prev, p_prev, v_prev, ~, plf_prev, ~, prf_prev, ~, ~] = Estimation.OCEKF.State.extract(obj.x_prev, obj.ba0, obj.bg0);
        ...
        function obj = updateState(obj, deltaY, H, R)
            ...
            K = (obj.P*H')/S;
            deltaX = K*deltaY;
            [q, p, v, ql, pl, qr, pr, ba, bg] = Estimation.OCEKF.State.extract(obj.x, obj.ba0, obj.bg0);
            [dq, dp, dv, dql, dpl, dqr, dpr, dba, dbg] = Estimation.OCEKF.State.extractLinearizedState(deltaX);
            ...
            if (obj.options.enable_bias_estimation)
                ba_upd = ba + dba;
                bg_upd = bg + dbg;
                obj.x = Estimation.OCEKF.State.constructwithbias(q_upd, p_upd, v_upd, ...
                    ql_upd, pl_upd, ...
                    qr_upd, pr_upd, ...
                    ba_upd, bg_upd);
            else
                obj.x = Estimation.OCEKF.State.constructwithoutbias(q_upd, p_upd, v_upd, ...
                    ql_upd, pl_upd, ...
                    qr_upd, pr_upd);
            end
        ...
        function obj = updateKinematics(obj, encoders, contacts, dt)
            [q, p, ~, ql, pl, qr, pr, ~, ~] = Estimation.OCEKF.State.extract(obj.x, obj.ba0, obj.bg0);
