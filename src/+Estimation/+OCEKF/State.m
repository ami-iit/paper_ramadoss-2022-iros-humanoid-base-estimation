classdef State
    methods (Static)
        ...
        function Psub = extractSubBlockVar(P, state)
            type = 'linearized';
            estimate_bias = true;
            range = Estimation.OCEKF.State.getRange(state, type, estimate_bias);
            Psub = P(range, range);
        end
        ...
        function [Pbrot, Ppos, Pvel, Plfpos, Plfrot, Prfpos, Prfrot, Pba, Pbg] = extractStateVarSubBlockEvolutions(Ptraj, estimate_bias)
            ...
            for iter_idx = 1: length(Ptraj)
                Pbrot(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'base_rot');
                Ppos(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'base_pos');
                Pvel(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'base_vel');
                Plfpos(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'lf_pos');
                Plfrot(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'lf_rot');
                Prfpos(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'rf_pos');
                Prfrot(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'rf_rot');
                if (estimate_bias)
                    Pba(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'bias_acc');
                    Pbg(iter_idx, :, :) = Estimation.OCEKF.State.extractSubBlockVar(squeeze(Ptraj{iter_idx}), 'bias_gyro');
                end
            end
        end
