classdef Filter < handle
    ...
    methods
        ...
        function obj = setup(obj, prior_dev, sensors_dev, model_comp, options)
            arguments
                obj Estimation.DILIGENT_KIO_RIE.Filter;
        ...
        function [X, P, BaseLinkState, DebugOut, obj] = advance(obj, t, omega, alpha, encoders, contacts)
            ...
            [R, p, v, ~, ~, ~, ~, ~, bg] = Estimation.DILIGENT_KIO_RIE.State.extract(obj.X);
            q = Utils.rot2quat(R);
    ...
    methods (Access = private)
        function obj = predictState(obj, omega, alpha, contact, dt)
            [R, ~, v, ~, ~, ~, ~, ba, bg] = Estimation.DILIGENT_KIO_RIE.State.extract(obj.X);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%
            XPrior = obj.X;
            Omega_lifted = Estimation.DILIGENT_KIO_RIE.State.exphat(Omega);   
            obj.X = Estimation.DILIGENT_KIO_RIE.State.compose(XPrior, Omega_lifted);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            AdjX = Estimation.DILIGENT_KIO_RIE.State.AdjointMatrix(XPrior);
            leftJacobianOmega = Estimation.DILIGENT_KIO_RIE.State.leftJacobian(Omega);
            dimP = size(AdjX, 1);
        ...
        function obj = updateState(obj, deltaY, H, R)
            ...
            deltaX_lifted = Estimation.DILIGENT_KIO_RIE.State.exphat(deltaX);

            obj.X = Estimation.DILIGENT_KIO_RIE.State.compose(deltaX_lifted, obj.X);
            
            I = eye(size(obj.P));
            leftJacobiandeltaX = Estimation.DILIGENT_KIO_RIE.State.leftJacobian(deltaX);
        ...
        function obj = updateKinematics(obj, encoders, contacts, dt)
            [A_R_IMU, p, ~, A_R_LF, pl, A_R_RF, pr, ~, ~] = Estimation.DILIGENT_KIO_RIE.State.extract(obj.X);
