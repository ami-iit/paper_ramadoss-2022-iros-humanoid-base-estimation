classdef Filter < handle
    ...
    methods
        ...
        function obj = setup(obj, prior_dev, sensors_dev, model_comp, options)
            arguments
                obj Estimation.CODILIGENT_KIO_RIE.Filter;
        ...
        function [X, P, BaseLinkState, DebugOut, obj] = advance(obj, t, omega, alpha, encoders, contacts)
            ...
            [R, p, v, ~, ~, ~, ~, ~, bg] = Estimation.CODILIGENT_KIO_RIE.State.extract(obj.X);
            q = Utils.rot2quat(R);
    ...
    methods (Access = private)
        function obj = predictState(obj, omega, alpha, contact, dt)
            [R, p, v, Zl, pl, Zr, pr, ba, bg] = Estimation.CODILIGENT_KIO_RIE.State.extract(obj.X);
...
%             pl_pred = contact(1)*pl + (1 - contact(1))*pl_off;
            ...
            Fc = [zeros(3)    zeros(3)      eye(3)  zeros(3,12); ...
                  zeros(3)    zeros(3)    zeros(3)  zeros(3,12); ...
                  zeros(3)      gCross    zeros(3)  zeros(3,12); ...
            ...
            if obj.options.enable_bias_estimation
                ...
                Fc(1:9, end-5:end) = [   zeros(3)  -pCross*R; ...
                                         zeros(3)         -R; ...
                                              -R   -vCross*R];
            end
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            dimWoBias = 21;
            Lc = Estimation.CODILIGENT_KIO_RIE.State.AdjointMatrix(obj.X);
            ...
            obj.X = Estimation.CODILIGENT_KIO_RIE.State.construct(R_pred, p_pred, v_pred, ...
                                                         Zl_pred, pl_pred,  ...
                                                         Zr_pred, pr_pred, ...
                                                         ba_pred, bg_pred, ...
        ...
        function obj = updateState(obj, deltaY, H, R)
            ...
            deltaX_lifted = Estimation.CODILIGENT_KIO_RIE.State.exphat(deltaX);

            obj.X = Estimation.CODILIGENT_KIO_RIE.State.compose(deltaX_lifted, obj.X);
            
            I = eye(size(obj.P));
            leftJacobiandeltaX = Estimation.CODILIGENT_KIO_RIE.State.leftJacobian(deltaX);
        ...
        function obj = updateKinematics(obj, encoders, contacts, dt)
            [A_R_IMU, p, ~, A_R_LF, pl, A_R_RF, pr, ~, ~] = Estimation.CODILIGENT_KIO_RIE.State.extract(obj.X);
