classdef Filter < handle
    ...
    methods
        ...
        function obj = setup(obj, prior_dev, sensors_dev, model_comp, options)
            arguments
                obj Estimation.DILIGENT_KIO.Filter;
        ...
        function [X, P, BaseLinkState, DebugOut, obj] = advance(obj, t, omega, alpha, encoders, contacts)
            ...
            [R, p, v, ~, ~, ~, ~, ~, bg] = Estimation.DILIGENT_KIO.State.extract(obj.X);
            q = Utils.rot2quat(R);
    ...
    methods (Access = private)
        function obj = predictState(obj, omega, alpha, contact, dt)
            [R, ~, v, ~, ~, ~, ~, ba, bg] = Estimation.DILIGENT_KIO.State.extract(obj.X);
            ...
            %%--propagate the mean
            %%%%%%%%%%%%%%%%%%%%%%%
            Omega_lifted = Estimation.DILIGENT_KIO.State.exphat(Omega);   
            obj.X = Estimation.DILIGENT_KIO.State.compose(obj.X, Omega_lifted);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            OmegaLiftedInverse = Estimation.DILIGENT_KIO.State.exphat(-Omega);

            AdjOmegaLiftedInverse = Estimation.DILIGENT_KIO.State.AdjointMatrix(OmegaLiftedInverse);
            rightJacobianOmega = Estimation.DILIGENT_KIO.State.rightJacobian(Omega);
        ...
        function obj = updateState(obj, deltaY, H, R)
            ...
            deltaX_lifted = Estimation.DILIGENT_KIO.State.exphat(deltaX);

            obj.X = Estimation.DILIGENT_KIO.State.compose(obj.X, deltaX_lifted);
            
            I = eye(size(obj.P));
            rightJacobiandeltaX = Estimation.DILIGENT_KIO.State.rightJacobian(deltaX);
        ...
        function obj = updateKinematics(obj, encoders, contacts, dt)
            [A_R_IMU, p, ~, A_R_LF, pl, A_R_RF, pr, ~, ~] = Estimation.DILIGENT_KIO.State.extract(obj.X);
