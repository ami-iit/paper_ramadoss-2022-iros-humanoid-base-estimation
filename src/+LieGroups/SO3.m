...
%             assert(isequal(S, -S'), 'input not a skew symmetric matrix')
        ...
        function Rout = sampleConcentratedGaussian(rpy_mean_rad, sigma_tangent_space)
            Rmean = LieGroups.SO3.fromRPY(rpy_mean_rad);
...
%                 end
%             end
            epsilon = randn*sigma_tangent_space;
        ...
        function Rmean = fromRPY(rpy_mean_rad)
            roll = rpy_mean_rad(1);
...
%         view(3);
%         hold on;
%         iDynTreeWrappers.plotFrame([Rarray{1} zeros(3,1); zeros(1, 3) 1], 0.5, 1, 1); 
%         iDynTreeWrappers.plotFrame([Rarray{2} zeros(3,1); zeros(1, 3) 1], 0.25, 2.5, 1);
%         iDynTreeWrappers.plotFrame([meanR zeros(3,1); zeros(1, 3) 1], 0.125, 5, 1);
        end
