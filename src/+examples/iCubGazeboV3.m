...
%% load dataset
experiment_name = 'gazebo-v3-walking'; 
experiment.mat_file = ['./resources/iCubGazeboV3/' experiment_name '.mat'];

load(experiment.mat_file);
experiment.robot_name='iCubGazeboV3'; %% Name of the robot
experiment.model_path = './resources/iCubGazeboV3/';  %% Path to the robot model
...
lfootLink = 'l_foot_rear';
rfootLink = 'r_foot_rear';
baseLink = 'chest';
baseLinkIMU = 'chest_imu_acc_1x1';
...
%% setup priors
priors = Config.iCubGazeboV3.configMatlabPriors();

%% setup sensor noise
sensors_dev = Config.iCubGazeboV3.configMatlabSensorDev(options.nr_joints_est);

%% setup schmitt trigger
[leftSchmittParams, rightSchmittParams] = Config.iCubGazeboV3.configSchmittParams;
...
figure
for idx =1:3
    plot(estBaseTime(startIter:endIter), imuAcc(startIter:endIter, idx), 'LineWidth', 2)
    hold on
end
xlim([estBaseTime(startIter) estBaseTime(endIter)])
set(gca,'FontSize',20, 'FontWeight', 'Bold')
